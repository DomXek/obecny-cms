...
export default async function NewsWidget({ tenantId, basePath = '' }: { tenantId?: string | null; basePath?: string }) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <Link href={`${basePath}/aktuality`} className="text-xs text-blue-600 hover:underline">Všetky →</Link>
      </div>
      {items.length === 0 ? (
        ...
        <ul className="space-y-3 flex-1 overflow-auto">
          {items.map(item => (
            <li key={item.id} className="border-b border-gray-100 pb-3 last:border-0 last:pb-0">
              <Link href={`${basePath}/aktuality/${item.slug}`} className="group">
